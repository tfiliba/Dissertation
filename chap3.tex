...
In this chapter we discuss work related to this work in the fields of Radio Astronomy and Electrical Engineering.
...
The work in electrical engineering focuses on design generation, explaining what type of heterogeneous design was possible before the work, and why it is not sufficient to solve the radio astronomy instrumentation design problem.
...
While this design might be reasonable when a pulsar is readily available, the software implementation is very costly to build from scratch.
...
Although the kernel itself is difficult to modify without GPU expertise, the implementation of the package in CUDA ensures that users without optimization experience are able to use the tool to implement large-n correlators.
...
SERENDIP, or the Search for Extraterrestrial Radio Emissions from Nearby Developed Intelligent Populations, aims to find extra terrestrial intelligence by analyzing radio signals \cite{2010LPICo1538.5378S}.
...
The picture on the right shows the beam multiplexor, and the left picture shows the IBOB and BEE2 boards that were used to develop the instrument.
...
The user can update the design simply by changing a parameter, clicking ok, and recompiling the design.
...
This design has been used in pulsar processors that have been deployed at the Parkes Observatory, Nançay Decimetric Radio Telescope, and Effelsberg Radio Telescope and aided in the discovery of a new astronomical object, a diamond planet \cite{Bailes:2011iy}.
...
Much of the work in Electrical Engineering relating to this kind of mapping use a fixed hardware model.
