...
Since it would be infeasible to channelize a 200MHz into 128 million channels using a single filter bank, the SERENDIP V.v spectrometer uses the high resolution architecture described in Chapter \ref{chap:Related Work}. %TODO: add reference to diagram
...
The book Interferometery and Synthesis Imaging by Thomson, Moran and Swenson \cite{Thompson:1986ww} gives a detailed description of how the end to end synthesis imaging process works, but in this work we will only focus on the real time computation, the cores-correlation.
