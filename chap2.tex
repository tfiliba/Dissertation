...
$$H_i(x) = FFT(f[n-n_{fi}]\ast b_{fi} + g[n-n_{gi}]\ast b_{gi})$$
...
In the case where many beams are required, each beam needs a different FIR filter for every antenna, and a separate FFT. 
This is called FB beamforming because the FFT operations (F) occur before the delay (B). 
...
In this case, there is an FFT for each antenna, rather than one FFT per beam. 
