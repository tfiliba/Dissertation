...
\begin{figure}[ht!]
  \centering
    \includegraphics[width=0.85\textwidth]{Images/C6/pfb_gpu_bench.png}
...
Note that the ROACH 3 hasn't been built, so the dollar cost is unknown but we can estimate the power consumption.
...
More simply, if the platforms available are a server and ROACH and we are placing FIR blocks all the FIR blocks must be placed on a ROACH or all the FIR blocks must be placed on a server.
...
ILP design optimization will become infeasible if there are many blocks that require a small amount of resources.
If the design is symmetric, as is often the case in radio astronomy, small blocks on the same type will end up in groups on the same board.
