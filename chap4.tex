...
First, as with most instruments, the astronomer must define the sky bandwidth, defined in MHz.
...
The sky bandwidth, integration time, and number of antennas are defined in the same way as the regular spectrometer. 

The spectral resolution is defined differently, because both the coarse and fine resolutions need to be defined. 
First, the coarse resolution defines how many channels the whole sky bandwidth should be broken up into.
Then, the fine resolution defines how many channels each coarse channel is broken into.
Both can be described in MHz per channel or by just giving the number of channels.
...
\begin{figure}[ht!]
  ...
  \textit{
  A general dataflow model for a spectrometer. 
  This model can be applied to any spectrometer, as the spectrometer parameters do no affect how many computational blocks are required. 
...
\begin{figure}[ht!]
  ...
  Like the previous FFT, the data comes in through an ADC, is filtered and then channelized using a coarse FFT.
...
\begin{figure}[ht!]
  ...
  \caption[ORCA Toolflow]{ORCA Toolflow Mapping}
  \label{fig: C4/toolflow_horizontal_s3.pdf}
\end{figure}

Once the dataflow and the computational blocks are defined, ORCA must determine how to place each computational block into hardware. 
In the mapping stage, ORCA determines what type of hardware should be used for each block, while minimizing the total cost of the hardware (in dollars, watts, or another used-defined metric).
...
The constraints are used to determine a valid mapping, for example total bandwidth mapped to a link must be less than total link bandwidth.
...
Unfortunately, the advantages of the ILP comes with a high cost, namely that the ILP is NP-Hard to solve optimally.
%TODO: ref
...
Because the runtime for the ORCA mapper needs to be within a few hours, we use a number of techniques to reduce the MILP runtime.
...
If that doesn't work, then it becomes necessary to modify the ILP.
...
Chapter \ref{chap:Algorithm Partitioning} goes into more detail on how the performance models are used as well as how the MILP is specified and optimized to achieve a feasible runtime. 
