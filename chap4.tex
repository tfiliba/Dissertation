...
The instrument parameters used when an instrument object is instantiated.
For example, to create the instrument specified on the sticky note in Figure \ref{fig: C4/instrument_description.pdf} would simply require the following function call (variables are defined for clarity):
...
Figure \ref{fig: C4/dataflow_model.pdf} shows an abstract example of this type of dataflow.
...
These blocks are similar to the set of blocks provided by the CASPER library or CUFFT, allowing for parameterization, but unlike the afore mentioned libraries, these blocks do not necessarily include an implementation of the function they claim to provide.
Instead, each block provides some method to asses the performance of that function on each supported platform.
...
In order to reduce the total number of connections that need to be defined, blocks that receive data from and sends data to the same set of block are put into a group.
...
The constructor for the CBlock has the user define the number of blocks in the group, and the input and output connections, indicating both the blocks that it is communicating with, the connection type, and the connection bandwidth.
...
The icon in Figure \ref{fig: C4/mapped_dataflow.pdf} represents an abstract mapped dataflow. 
